...
import { born, glow, bornPrefix, onTick, GLOW_GROWTH } from './clock.mjs';

/* Layer files live at the root of the wafer; this iteration sits two folders down. */
const ROOT = '../../';
const atRoot = p => /^[a-z]+:|^\//i.test(p) ? p : ROOT + p;
  ...
  if (on && !s.doc && !s.loading) {
    ...
    try {
      if (l.tiles) s.doc = await hydrateTiles(l, s);
        ...
        const r = await fetch(atRoot(l.file), { cache: 'default' });
...
async function hydrateTiles(l, s) {
  const r = await fetch(atRoot(l.tiles), { cache: 'default' });
  ...
  const parts = await Promise.all(bands.map(async b => {
    const t = await fetch(atRoot(b.file), { cache: 'default' });
...
function drawOverlay() {
  ...
  for (const [l, doc] of visibleLayers()) {
    ...
    for (const f of doc.features) {
      const g = f.geometry;
      if (g.type === 'Point') {
        if (!born(g.key)) continue;
        const [x, y] = S(place(g.key));
        if (x < -2 || y < -2 || x > v.w + 2 || y > v.h + 2) continue;
        const w = glow(g.key), h = 1 + GLOW_GROWTH * w;
        if (w > 0) { ctx.save(); ctx.globalAlpha = 1; ctx.shadowColor = l.colour; ctx.shadowBlur = 8 * w; }
        ctx.fillRect(x - h, y - h, 2 * h, 2 * h);
        if (w > 0) ctx.restore();
      } else if (g.type === 'LineString') {
        /* keys are in numbering order, i.e. in time: draw only the born prefix */
        const last = bornPrefix(g.keys);
        if (last < 0) continue;
        ctx.beginPath();
        for (let i = 0; i <= last; i++) { const [x, y] = S(place(g.keys[i])); i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); }
        ctx.stroke();
        const tip = g.keys[last], w = glow(tip);
        if (w > 0) {
          const [x, y] = S(place(tip));
          ctx.save(); ctx.globalAlpha = w; ctx.shadowColor = l.colour; ctx.shadowBlur = 10 * w;
          ctx.beginPath(); ctx.arc(x, y, 2 * (1 + GLOW_GROWTH * w), 0, 6.2832); ctx.fill(); ctx.restore();
        }
      }
    }
    ctx.globalAlpha = 1;
...
(async () => {
  try {
    const r = await fetch(atRoot('layers/manifest.json'), { cache: 'no-cache' });
