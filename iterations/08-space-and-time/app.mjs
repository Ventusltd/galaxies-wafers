...
import { clock, setT, onTick, dateIndex, GLOW_SHARE, GLOW_GROWTH } from './clock.mjs';
...
const snapshot = () => Object.freeze({ x: view.x, y: view.y, zoom: view.zoom, w: view.w, h: view.h, dpr: view.dpr, t: clock.t });
...
async function tier2() {
  ...
  dateOf = dateIndex(families, famLines);
  paintClock();
...
in vec2 a_pos; in float a_len; in float a_fam; in float a_key;
uniform float u_t; uniform float u_glowShare; uniform float u_glowGrowth;
out float v_glow; out float v_born;
...
void main(){
  ...
  v_born = a_key <= u_t ? 1.0 : 0.0;
  v_glow = v_born * clamp(1.0 - (u_t - a_key) / max(u_t * u_glowShare, 1.0), 0.0, 1.0);
  base *= 1.0 + u_glowGrowth * v_glow;
  gl_PointSize = v_born * clamp(base * sqrt(u_zoom) * u_dpr, 1.0, 26.0 * u_dpr);
  v_fam = a_fam; v_len = a_len; v_focus = 0.0;
}`;

const FS = `#version 300 es
precision highp float;
in float v_fam; in float v_len; in float v_focus; in float v_glow; in float v_born;
out vec4 o;
void main(){
  vec2 d = gl_PointCoord - 0.5;
  float r = length(d);
  if (r > 0.5 || v_born < 0.5) discard;
  float edge = smoothstep(0.5, 0.42, r);
  vec3 carried = vec3(0.84, 0.87, 0.94);
  vec3 alone   = vec3(0.30, 0.34, 0.44);
  vec3 c = mix(mix(alone, carried, v_fam), vec3(1.0, 0.93, 0.62), v_glow);
  o = vec4(c, edge * mix(0.30 + 0.70 * v_fam, 1.0, v_glow));
}`;
...
function buildGL() {
  ...
  gl.useProgram(prog);
  for (const u of ['u_res', 'u_cam', 'u_zoom', 'u_dpr', 'u_focusKey', 'u_t', 'u_glowShare', 'u_glowGrowth']) loc[u] = gl.getUniformLocation(prog, u);
  ...
  put(U.keys, 'a_key', 1, gl.UNSIGNED_INT, false);
  gl.enable(gl.BLEND);
...
function render() {
  if (gl) {
    ...
    gl.uniform1f(loc.u_t, clock.t);
    gl.uniform1f(loc.u_glowShare, GLOW_SHARE);
    gl.uniform1f(loc.u_glowGrowth, GLOW_GROWTH);
    ...
    for (let i = 0; i < U.n; i += step) {
      if (U.keys[i] > clock.t) break;          /* keys are sorted: the rest are not yet born */
...
/* ── the clock: time as depth ────────────────────────────────────────────── */

const SWEEP_SECONDS = 60;  /* with no ?rate=, PLAY crosses the whole key range in a minute */
let dateOf = null;         /* built by tier 2 from families.json; null until then */
let playing = false, lastFrame = 0, rate = 0;

function paintClock() {
  $('tslider').value = String(clock.t);
  $('tkey').textContent = `key ${fmt(clock.t)} of ${fmt(clock.max)}`;
  if (!dateOf) { $('tdate').textContent = 'dates loading…'; return; }
...
function writeT() {
  ...
  u.searchParams.set('t', String(clock.t));
  history.replaceState(history.state, '', u.pathname + u.search.replace(/%2C/gi, ',') + u.hash);
}

function tickPlay(now) {
  if (!playing) return;
  if (lastFrame) setT(clock.t + rate * (now - lastFrame) / 1000);
  lastFrame = now;
  if (clock.t >= clock.max) { playing = false; $('tplay').textContent = 'PLAY'; writeT(); return; }
  requestAnimationFrame(tickPlay);
}

function startClock() {
  clock.max = U.meta.max;
  const q = new URLSearchParams(location.search);
  const pt = parseKey(q.get('t') ?? '');
  const pr = parseKey(q.get('rate') ?? '');
  rate = pr.ok ? pr.key : Math.max(1, Math.round(clock.max / SWEEP_SECONDS));
  const s = $('tslider');
  s.min = '1'; s.max = String(clock.max); s.step = '1';
  clock.t = pt.ok ? Math.min(pt.key, clock.max) : clock.max;
  onTick(() => { paintClock(); draw(); });
  s.addEventListener('input', () => setT(Number(s.value)));
  s.addEventListener('change', writeT);
  $('tplay').addEventListener('click', () => {
    playing = !playing;
    $('tplay').textContent = playing ? 'PAUSE' : 'PLAY';
    if (playing) { if (clock.t >= clock.max) setT(1); lastFrame = 0; requestAnimationFrame(tickPlay); }
    else writeT();
  });
  paintClock();
}
