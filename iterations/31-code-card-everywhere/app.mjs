/* Code Card Everywhere (iteration 31): Dark Pixels, where any tap on the wafer
 * opens a code card (card.mjs). Differences from iteration 21's app.mjs: card.mjs
 * is imported and bound to the loaded data, and a tap no longer opens the line
 * panel; it hands the tap point to the card. Everything else is unchanged.
 *
 * Dark Pixels: the Line Wafer with its ground switched off.
 *
 * This is the root wafer's app.mjs, copied because the root reads its data by
 * relative path. Three things differ and nothing else: the import path of
 * lib.mjs, the fragment shader (every line point is drawn near-black, faint
 * enough that the spiral still reads), and drawMarks() only resizes its canvas
 * when the size changes. The layers that wake points are in layers-panel.mjs.
 *
 * What follows is the root's own description, unchanged.
 *
 * The Line Wafer.
 *
...
import { bindWafer, cardTap } from './card.mjs';
...
async function tier2() {
  ...
  window.dispatchEvent(new Event('wafer:families'));   /* iteration 31: a card waiting on the index resumes */
...
function gestures() {
  ...
  const up = e => {
    if (pts.size === 1 && down && moved < 7) {
      /* iteration 31: every tap opens a code card, on a line, a route or empty ground */
...
function writeURL(a, b) {
  ...
  /* Dark Pixels: keep the ticked layers in the link when a line is flown to */
  const keep = new URLSearchParams(location.search).get('layers');
  if (b) q.set('to', String(b));
  if (keep) q.set('layers', keep);
  history.replaceState(null, '', q.toString() ? '?' + q.toString().replace(/%2C/gi, ',') : location.pathname);
}
