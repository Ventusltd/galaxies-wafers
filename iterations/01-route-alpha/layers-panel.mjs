...
/* ROUTE ALPHA (iteration 01). Copied from the root layers-panel.mjs; the only
   changes are the ROOT prefix for fetches, the route alpha rule below, and the
   density legend line. Every LineString route is stroked at width ROUTE_WIDTH
   with alpha = scale / sqrt(features in its layer), clamped to 1, so a dense
   module goes translucent and a sparse one stays bright. `scale` is ?alpha=,
   clamped to [ALPHA_MIN, 1]; absent or unreadable, it is 1. */
const ROOT = '../../';
const ROUTE_WIDTH = 0.6;
const ALPHA_MIN = 0.05;
const alphaScale = (() => {
  const raw = new URL(location.href).searchParams.get('alpha');
  const v = raw === null ? NaN : Number(raw);
  return Number.isFinite(v) ? Math.min(1, Math.max(ALPHA_MIN, v)) : 1;
})();
const routeAlpha = n => Math.min(1, alphaScale / Math.sqrt(Math.max(1, n)));
...
function densityLegend() {
  const on = [...visibleLayers()].filter(([, d]) => d.features.some(f => f.geometry.type === 'LineString'));
  const each = on.map(([l, d]) => `${esc(l.id)} ${d.features.length} → ${routeAlpha(d.features.length).toFixed(3)}`).join(' · ');
  return `<div class="lgroup">DENSITY LEGEND</div><div class="lnote" id="densityLegend">route alpha = ${alphaScale} ÷ √(features in layer), max 1 · width ${ROUTE_WIDTH} px · ?alpha= scales it (${ALPHA_MIN}–1)${each ? ' · ' + each : ''}</div>`;
}
...
function renderPanel() {
  ...
  ).join('') + densityLegend() + `<div class="lnote">substrate ${esc(manifest.substrate)} · frozen · layers built ${esc(manifest.built_utc)}</div>`;
...
function drawOverlay() {
  ...
  for (const [l, doc] of visibleLayers()) {
    ...
    const pointAlpha = l.draws === 'lines' ? 0.55 : 0.8;
    const lineAlpha = routeAlpha(doc.features.length);
    ctx.lineWidth = ROUTE_WIDTH;
    for (const f of doc.features) {
      const g = f.geometry;
      ctx.globalAlpha = g.type === 'LineString' ? lineAlpha : pointAlpha;
      if (g.type === 'Point') {
