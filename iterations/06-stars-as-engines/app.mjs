...
/* Where a number is drawn: the wafer law by default, or the engine chosen in engines.mjs. */
const placeOne = engines.placeKey;
...
let gl = null, prog = null, loc = {}, vao = null, ctx2d = null, posBuf = null;
...
function buildGL() {
  ...
  posBuf = put(U.pos, 'a_pos', 2, gl.FLOAT, false);
...
function writeURL(a, b) {
  /* Keeps ?engine= and ?layers=; only line and to are this function's to write. */
  const q = new URLSearchParams(location.search);
...
(async function start() {
  ...
  engines.attach(U, () => {
    if (gl && posBuf) { gl.bindBuffer(gl.ARRAY_BUFFER, posBuf); gl.bufferSubData(gl.ARRAY_BUFFER, 0, U.pos); }
    render();
  });
