/* Apps, Layers, Substrate (iteration 26). This file is iteration 21's app.mjs
 * (the dark substrate) with one change: writeURL keeps every other parameter.
 *
...
function writeURL(a, b) {
  /* Apps, Layers, Substrate: every other parameter (app, depth, layers) is kept */
  const q = new URLSearchParams(location.search);
  if (a) q.set('line', String(a)); else q.delete('line');
  if (b) q.set('to', String(b)); else q.delete('to');
  history.replaceState(history.state, '', q.toString() ? '?' + q.toString().replace(/%2C/gi, ',').replace(/%7E/gi, '~') : location.pathname);
}
