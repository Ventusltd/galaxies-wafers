...
/* ITERATION 02 — TUNNELS. This copy reads its manifest from this folder (the root
   manifest plus the tunnels layer) and resolves every layer file against the repo
   root, so the root layers/ are shared, not copied. A layer whose manifest entry
   says draws: "arcs" is drawn as a quadratic arc bowing toward the wafer centre. */
const ROOT = new URL('../../', import.meta.url);
const fromRoot = path => new URL(path, ROOT).href;
const ARC_PULL = 0.35;     /* the control point is the midpoint moved this far toward the origin */
const ARC_WIDTH = 0.5, ARC_ALPHA = 0.35, ARC_STEPS = 24;
const arcControl = ([ax, ay], [bx, by]) => [(ax + bx) / 2 * (1 - ARC_PULL), (ay + by) / 2 * (1 - ARC_PULL)];
  ...
  if (on && !s.doc && !s.loading) {
    ...
    try {
      if (l.tiles) s.doc = await hydrateTiles(l, s);
        ...
        const r = await fetch(fromRoot(l.file), { cache: 'default' });
...
async function hydrateTiles(l, s) {
  const r = await fetch(fromRoot(l.tiles), { cache: 'default' });
  ...
  const parts = await Promise.all(bands.map(async b => {
    const t = await fetch(fromRoot(b.file), { cache: 'default' });
...
function drawOverlay() {
  ...
  for (const [l, doc] of visibleLayers()) {
    ...
    for (const f of doc.features) {
      ...
      } else if (g.type === 'LineString' && l.draws === 'arcs' && g.keys.length === 2) {
        const a = place(g.keys[0]), b = place(g.keys[1]);
        const [ax, ay] = S(a), [bx, by] = S(b), [cx, cy] = S(arcControl(a, b));
        ctx.globalAlpha = ARC_ALPHA; ctx.lineWidth = ARC_WIDTH;
  ...
  for (const [l, doc] of visibleLayers()) {
    for (const f of doc.features) {
      ...
      if (g.type === 'Point') {
        ...
        const a = place(g.keys[0]), b = place(g.keys[1]), c = arcControl(a, b);
        let prev = toScreen(v, a);
        for (let i = 1; i <= ARC_STEPS; i++) {
          const t = i / ARC_STEPS, u = 1 - t;
          const q = toScreen(v, [u * u * a[0] + 2 * u * t * c[0] + t * t * b[0], u * u * a[1] + 2 * u * t * c[1] + t * t * b[1]]);
          d = Math.min(d, segDist2(cx, cy, prev, q)); prev = q;
        }
...
function inspect(hit) {
  ...
  const dl = node('dl');
  const p = f.properties || {};
  if (l.draws === 'arcs' && p.early && p.late) {
    where.textContent = `shared line ${p.key}`;
    dl.append(node('dt', 'from'), node('dd', `${p.early.name} (family ${p.early.family}, first line ${p.early.first})`),
              node('dt', 'to'), node('dd', `${p.late.name} (family ${p.late.family}, first line ${p.late.first})`));
  }
...
(async () => {
  try {
    const r = await fetch(new URL('manifest.json', import.meta.url).href, { cache: 'no-cache' });
