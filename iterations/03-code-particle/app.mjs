...
function paintPanel(key) {
  ...
  p.innerHTML = body;
  $('panel').hidden = false;
  /* iteration 03: the code particle — the tapped line opens its real code block, lazily. */
  if (fams && fams.length) import('./code-particle.mjs').then(m => m.openCode(fams.map(f => U.families[f]), p));
}
