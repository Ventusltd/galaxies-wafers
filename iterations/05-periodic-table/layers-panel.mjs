...
/* This copy lives two folders down; every layer path in the manifest is relative to the repository root. */
...
function renderPanel() {
  if (!manifest) return;
  document.dispatchEvent(new CustomEvent('layers-change'));
...
async function toggle(l, on) {
  ...
  if (on && !s.doc && !s.loading) {
    ...
    s.promise = (async () => { try {
      if (l.tiles) s.doc = await hydrateTiles(l, s);
        ...
        const r = await fetch(at(l.file), { cache: 'default' });
  ...
  if (on && s.promise) await s.promise;
...
async function hydrateTiles(l, s) {
  const r = await fetch(at(l.tiles), { cache: 'default' });
  ...
  const parts = await Promise.all(bands.map(async b => {
    const t = await fetch(at(b.file), { cache: 'default' });
...
(async () => {
  try {
    const r = await fetch(at('layers/manifest.json'), { cache: 'no-cache' });
    ...
    renderPanel();
    /* The one hook the periodic table uses: hydrate a layer by id, read its state,
       and put the ?layers= list back after something else rewrote the URL. */
    window.__layers = Object.freeze({
      has: id => state.has(id),
      stateOf: id => state.get(id),
      hydrate: id => { const l = manifest.layers.find(x => x.id === id); return l ? toggle(l, true) : Promise.resolve(); },
      writeURL: writeLayersToURL,
    });
    document.dispatchEvent(new CustomEvent('layers-ready'));
    readLayersFromURL();
  } catch (e) {
