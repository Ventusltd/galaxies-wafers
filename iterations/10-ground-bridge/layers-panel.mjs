...
/* THE GROUND BRIDGE (iteration 10). This copy runs in the left pane of
   iterations/10-ground-bridge/, two folders below the repository root, so every
   layer path is read from there. It also exposes one small hook, __layers, so
   the map pane can light module routes and hear which route was tapped. */
const ROOT = '../../';
...
function inspect(hit) {
  ...
  for (const f of bridge.onPick) { try { f(l.id, hit.feature); } catch (e) { console.warn('onPick listener failed:', e); } }
  box.hidden = false;
...
/* ── the bridge: a queue that hydrates module routes one at a time ───────── */
const bridge = { onPick: new Set() };
let queue = Promise.resolve();
window.__layers = Object.freeze({
  onPick: bridge.onPick,
  /* ids the manifest names, in manifest order */
  ids: () => (manifest?.layers || []).map(l => l.id),
  status: id => state.get(id)?.status ?? null,
  label: id => manifest?.layers.find(l => l.id === id)?.label ?? null,
  /* light exactly these layers (others the bridge lit are switched off); returns when all have settled */
  light(ids) {
    queue = queue.then(async () => {
      ...
      for (const l of manifest.layers) {
        const s = state.get(l.id);
        if (s?.on && s.bridged && !want.has(l.id)) { s.bridged = false; await toggle(l, false); }
      }
      for (const id of ids) {
        const l = manifest.layers.find(x => x.id === id);
        if (!l) continue;
        state.get(id).bridged = true;
        await toggle(l, true);
      }
      $('layersBody').hidden = false;
      if (ids.length) document.getElementById('L-' + ids[0])?.scrollIntoView({ block: 'start' });
    });
    return queue;
  }
});
