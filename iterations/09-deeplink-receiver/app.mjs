...
/* Iteration 09: the URL is read once, by the engine's parseDeepLink, in receiver.mjs. */
import { LINK } from './receiver.mjs';

/* The zoom the link asked for; applied on arrival, forgotten once the user flies elsewhere. */
let linkZoom = LINK.ok ? LINK.zoom : undefined;
...
function connect(ka, kb) {
  ...
  if (linkZoom !== undefined) view.zoom = linkZoom;
  draw();
}
...
/* The receiver. Everything comes from LINK, which is parseDeepLink(location.href).
   On ok:false the refusal card (receiver.mjs) quotes the engine's reason and
   the wafer stays at home; nothing is guessed. */
function readURL() {
  if (!LINK.ok) return;
  $('a').value = String(LINK.line);
  if (LINK.to !== undefined) { $('b').value = String(LINK.to); connect(LINK.line, LINK.to); }
  else { view.focus = LINK.line; paintPanel(LINK.line); flyTo(LINK.line, linkZoom); }
}
